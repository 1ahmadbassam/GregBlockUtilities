...
public class SteamSieve extends SteamMetaTileEntity {
    ...
    public ModularUI createUI(EntityPlayer player) {
        ...
        ModularUI.Builder builder =  ModularUI.builder(BRONZE_BACKGROUND_TEXTURE, 180, 230)
                ...
                .bindPlayerInventory(player.inventory, BRONZE_SLOT_BACKGROUND_TEXTURE, 8, 144)
                .widget(new SlotWidget(this.importItems, 0, 35, 25)
                        .setBackgroundTexture(BRONZE_SLOT_BACKGROUND_TEXTURE))
                .widget(new SlotWidget(this.importItems, 1, 53, 25)
                        .setBackgroundTexture(BRONZE_SLOT_BACKGROUND_TEXTURE))
                .widget(new ProgressWidget(workableHandler::getProgressPercent, 78, 26, 20, 18)
        ...
        for (int y = 0; y < 7; y++) {
            for (int x = 0; x < 4; x++) {
                builder.widget(new SlotWidget(this.exportItems, y * 4 + x, 100 + x * 18, 7 + y * 18, true, false).setBackgroundTexture(BRONZE_SLOT_BACKGROUND_TEXTURE));
            }
        }
